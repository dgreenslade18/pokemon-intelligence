...
export async function POST(request: NextRequest) {
  try {
    ...
    await sql`CREATE TABLE IF NOT EXISTS population_data (
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_population_card_name ON population_data(card_name)`
    await sql`CREATE INDEX IF NOT EXISTS idx_population_set_slug ON population_data(set_slug)`
    await sql`CREATE INDEX IF NOT EXISTS idx_population_service ON population_data(grading_service)`
    await sql`CREATE INDEX IF NOT EXISTS idx_population_updated ON population_data(last_updated)`
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await sql`${tableInfoQuery}`
